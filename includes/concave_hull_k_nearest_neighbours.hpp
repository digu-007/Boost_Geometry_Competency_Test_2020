...
static inline MultiPoint Solve(MultiPoint dataset, Factor k)
{
    ...
    bgi::rtree<point_type, bgi::rstar<32>> rt;
    BuildRTree(dataset, rt);
    ...
    while (boost::size(dataset))
    {
        ...
        rt.query(bgi::nearest(cur_point, cur_k), std::back_inserter(k_nearest));
        ...
        for (size_type i = 0; i < size1; ++i)
        {
            ...
            for (size_type j = size2 - 1; j > 0; --j)
            {
                linestring_type ls2{ans[j], ans[j - 1]};
                if (bg::intersects(ls1, ls2) and !(bg::touches(ls1, ls2)))
                {   
